...
function getPool() {
  ...
  try {
    ...
    console.error("Pool parse error:", e.message, "URL start:", url.substring(0, 50));
    return null;
  }
  return _pool;
}
...
export default async function handler(event) {
  ...
  try {
    ...
    if (!pool) {
      const envKeys = Object.keys(process.env).filter(function (k) {
        return k.includes("DATABASE") || k.includes("NEON") || k.includes("POSTGRES") || k.includes("PG");
      });
      return json({ error: "No DATABASE_URL found", envKeys: envKeys }, 500);
    }
  } catch (e) {
  ...
  try {
    ...
    if (method === "GET" && path === "workers") {
      return json(await q("SELECT * FROM workers ORDER BY type, name"));
    }
    if (method === "POST" && path === "workers") {
      if (!body.name) return err("Nom requis");
      return json(await q1(
        "INSERT INTO workers (name,agency,type,phone,badge,sched_in,sched_out) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *",
        [body.name, body.agency || "", body.type || "interim", body.phone || "", body.badge || "", body.schedIn || "08:00", body.schedOut || "16:00"]
      ), 201);
    }
    if (method === "PUT" && seg[0] === "workers" && seg[1]) {
      var id = parseInt(seg[1]);
      var r = await q1(
        "UPDATE workers SET name=COALESCE($1,name), agency=COALESCE($2,agency), type=COALESCE($3,type), phone=COALESCE($4,phone), badge=COALESCE($5,badge), sched_in=COALESCE($6,sched_in), sched_out=COALESCE($7,sched_out), updated_at=NOW() WHERE id=$8 RETURNING *",
    ...
    if (method === "POST" && path === "records") {
      ...
      return json(await q1(
        "INSERT INTO records (worker_id,worker_name,agency,date,arrival,departure,breaks) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *",
        [body.workerId, body.workerName || "", body.agency || "", body.date, body.arrival, body.departure || null, JSON.stringify(body.breaks || [])]
      ), 201);
    }
    if (method === "PUT" && seg[0] === "records" && seg[1]) {
      var rid = parseInt(seg[1]);
      var rec = await q1(
        "UPDATE records SET arrival=COALESCE($1,arrival), departure=$2, breaks=COALESCE($3,breaks), updated_at=NOW() WHERE id=$4 RETURNING *",
        [body.arrival, body.departure !== undefined ? body.departure : null, body.breaks ? JSON.stringify(body.breaks) : null, rid]
      );
      return rec ? json(rec) : err("Introuvable", 404);
    }
    ...
    if (method === "GET" && path === "postes") {
      return json(await q("SELECT * FROM postes ORDER BY name"));
    }
    ...
    if (method === "GET" && path === "qr-secret") {
      var row = await q1("SELECT value FROM settings WHERE key='qr_secret'");
      if (!row) {
        var s = genSecret();
        await q("INSERT INTO settings (key,value) VALUES ('qr_secret',$1)", [s]);
    ...
    if (method === "POST" && path === "qr-secret/regenerate") {
      var ns = genSecret();
      await q("INSERT INTO settings (key,value) VALUES ('qr_secret',$1) ON CONFLICT (key) DO UPDATE SET value=$1", [ns]);
      return json({ secret: ns });
    }
...
async function initDB() {
  ...
  await q("CREATE TABLE IF NOT EXISTS postes (id SERIAL PRIMARY KEY, name VARCHAR(255) NOT NULL, location VARCHAR(255) DEFAULT '', created_at TIMESTAMP DEFAULT NOW())");
  await q("CREATE TABLE IF NOT EXISTS settings (key VARCHAR(100) PRIMARY KEY, value TEXT NOT NULL)");
  var pwd = await q1("SELECT value FROM settings WHERE key='admin_password'");
  if (!pwd) await q("INSERT INTO settings (key,value) VALUES ('admin_password','admin')");
  var qrs = await q1("SELECT value FROM settings WHERE key='qr_secret'");
...
async function handleLogin(body) {
  ...
  var wid = body.workerId;
  if (mode === "admin") {
    var r = await q1("SELECT value FROM settings WHERE key='admin_password'");
    return (!r || r.value !== password) ? err("Mot de passe incorrect", 401) : json({ role: "admin" });
  }
  if (mode === "badge") {
    if (!badge) return err("Badge requis");
    var w = await q1("SELECT * FROM workers WHERE badge=$1 AND badge!=''", [badge]);
    return w ? json({ role: "employee", worker: w }) : err("Badge inconnu", 401);
  }
  if (mode === "name") {
    if (!wid) return err("workerId requis");
    var w2 = await q1("SELECT * FROM workers WHERE id=$1", [parseInt(wid)]);
    return w2 ? json({ role: "employee", worker: w2 }) : err("Introuvable", 401);
  }
...
async function handleScan(body) {
  var badge = body.badge;
  var wid = body.workerId;
  var qrData = body.qrData;
  var worker;
  if (badge) {
    worker = await q1("SELECT * FROM workers WHERE badge=$1 AND badge!=''", [badge]);
    if (!worker) return json({ action: "unknown_badge", badge: badge });
  } else if (wid) {
    worker = await q1("SELECT * FROM workers WHERE id=$1", [parseInt(wid)]);
    if (!worker) return err("Introuvable", 404);
  } else {
    return err("badge ou workerId requis");
  }

  if (qrData) {
    var qrRow = await q1("SELECT value FROM settings WHERE key='qr_secret'");
    var parts = qrData.split(":");
    if (parts.length !== 3 || parts[0] !== "iziship" || parts[1] !== ((qrRow && qrRow.value) || "")) {
  ...
  var recs = await q("SELECT * FROM records WHERE worker_id=$1 AND date=$2 ORDER BY id DESC LIMIT 1", [worker.id, today]);
  var rec = recs[0];

  if (!rec || rec.departure) {
    var nr = await q1(
      "INSERT INTO records (worker_id,worker_name,agency,date,arrival,breaks) VALUES ($1,$2,$3,$4,$5,'[]') RETURNING *",
      [worker.id, worker.name, worker.agency || "", today, timeNow]
    );
    return json({ action: "arrival", time: timeNow, worker: worker, record: nr });
  }
  ...
  if (lb && !lb.end) {
    lb.end = timeNow;
    var up = await q1("UPDATE records SET breaks=$1, updated_at=NOW() WHERE id=$2 RETURNING *", [JSON.stringify(breaks), rec.id]);
    return json({ action: "break_end", time: timeNow, worker: worker, record: up });
  }
