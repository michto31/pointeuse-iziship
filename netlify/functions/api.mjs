...
async function sql(query, params) {
  ...
  if (!res.ok) {
    ...
    throw new Error("DB error " + res.status + ": " + errBody.substring(0, 200));
  }
  ...
  var rows = (data.rows || []).map(function (row) {
    ...
    for (var i = 0; i < fields.length; i++) {
      var val = row[i];
      if (fields[i].dataTypeID === 114 || fields[i].dataTypeID === 3802) {
...
async function sql1(query, params) { var rows = await sql(query, params); return rows[0] || null; }
...
export default async function handler(event) {
  ...
  try {
    ...
    if (method === "POST" && path === "workers") {
      ...
      return json(await sql1("INSERT INTO workers (name,agency,type,phone,badge,sched_in,sched_out) VALUES ($1,$2,$3,$4,$5,$6,$7) RETURNING *",
    ...
    if (method === "PUT" && seg[0] === "workers" && seg[1]) {
      ...
      var r = await sql1("UPDATE workers SET name=COALESCE($1,name), agency=COALESCE($2,agency), type=COALESCE($3,type), phone=COALESCE($4,phone), badge=COALESCE($5,badge), sched_in=COALESCE($6,sched_in), sched_out=COALESCE($7,sched_out), updated_at=NOW() WHERE id=$8 RETURNING *",
    ...
    if (method === "DELETE" && seg[0] === "workers" && seg[1]) {
      await sql("DELETE FROM workers WHERE id=$1", [parseInt(seg[1])]);
      return json({ ok: true });
    }
    ...
    if (method === "GET" && path === "records") {
      ...
      var from = qs.from;
      var to = qs.to;
      if (date && workerId) return json(await sql("SELECT r.*, w.type as worker_type FROM records r JOIN workers w ON r.worker_id=w.id WHERE r.date=$1 AND r.worker_id=$2 ORDER BY r.arrival", [date, parseInt(workerId)]));
      ...
      if (from && to && workerId) return json(await sql("SELECT r.*, w.type as worker_type FROM records r JOIN workers w ON r.worker_id=w.id WHERE r.date>=$1 AND r.date<=$2 AND r.worker_id=$3 ORDER BY r.date, r.arrival", [from, to, parseInt(workerId)]));
      ...
      return json(await sql("SELECT r.*, w.type as worker_type FROM records r JOIN workers w ON r.worker_id=w.id WHERE r.date=$1 ORDER BY w.type, r.arrival", [today]));
    }
    if (method === "POST" && path === "records") {
      if (!body.workerId || !body.arrival) return err("workerId et arrival requis");
      return json(await sql1("INSERT INTO records (worker_id,worker_name,agency,date,arrival,departure,breaks) VALUES ($1,$2,$3,$4,$5,$6,$7::jsonb) RETURNING *",
    ...
    if (method === "PUT" && seg[0] === "records" && seg[1]) {
      ...
      return json(await sql1("UPDATE records SET arrival=COALESCE($1,arrival), departure=$2, breaks=COALESCE($3::jsonb,breaks), updated_at=NOW() WHERE id=$4 RETURNING *",
        [body.arrival, dep, brk, rid]), 200);
    }
    if (method === "DELETE" && seg[0] === "records" && seg[1]) {
      await sql("DELETE FROM records WHERE id=$1", [parseInt(seg[1])]);
      return json({ ok: true });
    }
    ...
    if (method === "POST" && path === "postes") {
      if (!body.name) return err("Nom requis");
      return json(await sql1("INSERT INTO postes (name,location) VALUES ($1,$2) RETURNING *", [body.name, body.location||""]), 201);
    }
    if (method === "DELETE" && seg[0] === "postes" && seg[1]) {
      await sql("DELETE FROM postes WHERE id=$1", [parseInt(seg[1])]);
      return json({ ok: true });
    }
    ...
    if (method === "POST" && path === "qr-secret/regenerate") {
      var s = genSecret();
      await sql("INSERT INTO settings (key,value) VALUES ('qr_secret',$1) ON CONFLICT (key) DO UPDATE SET value=$1", [s]);
...
async function initDB() {
  ...
  var pwd = await sql1("SELECT value FROM settings WHERE key='admin_password'");
  if (!pwd) await sql("INSERT INTO settings (key,value) VALUES ('admin_password','admin')");
  var qrs = await sql1("SELECT value FROM settings WHERE key='qr_secret'");
  if (!qrs) await sql("INSERT INTO settings (key,value) VALUES ('qr_secret',$1)", [genSecret()]);
...
async function handleLogin(body) {
  if (body.mode === "admin") {
    ...
    return (!r || r.value !== body.password) ? err("Mot de passe incorrect", 401) : json({ role: "admin" });
  }
  if (body.mode === "badge") {
...
async function handleScan(body) {
  var worker;
  if (body.badge) {
    worker = await sql1("SELECT * FROM workers WHERE badge=$1 AND badge!=''", [body.badge]);
    if (!worker) return json({ action: "unknown_badge", badge: body.badge });
  } else if (body.workerId) {
    worker = await sql1("SELECT * FROM workers WHERE id=$1", [parseInt(body.workerId)]);
    if (!worker) return err("Introuvable", 404);
  } else return err("badge ou workerId requis");

  if (body.qrData) {
    var qrRow = await sql1("SELECT value FROM settings WHERE key='qr_secret'");
  ...
  if (!rec || rec.departure) {
    var nr = await sql1("INSERT INTO records (worker_id,worker_name,agency,date,arrival,breaks) VALUES ($1,$2,$3,$4,$5,'[]'::jsonb) RETURNING *",
  ...
  if (lb && !lb.end) {
    lb.end = timeNow;
    var up = await sql1("UPDATE records SET breaks=$1::jsonb, updated_at=NOW() WHERE id=$2 RETURNING *", [JSON.stringify(breaks), rec.id]);
