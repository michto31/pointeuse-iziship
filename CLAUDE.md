...
One carve-out for the shared-tablet login flow: `GET /api/public/worker-names` returns just `[{id, name}]` (no badge/phone/agency). Rate-limited to 20 req/min/IP via an in-process `Map` — best-effort against scraping, resets on cold start. Do not broaden this endpoint.
